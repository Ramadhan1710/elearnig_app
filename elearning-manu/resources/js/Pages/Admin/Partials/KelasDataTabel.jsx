...
const KelasDataTable = ({
    ...
    const columns = [
        ...
        columnHelper.accessor("range_kelas", {
            cell: (info) => <span>{info.getValue()}</span>,
            header: "Kelas",
        }),
        ...
        }),
        {
            cell: (info) => (
                <div className="flex gap-1">
                    <Link
                        href={`/admin/kelas/${info.row.original.id}`}
                        className="bg-yellow-400 p-2 rounded hover:bg-amber-500 transition"
                    >
                        <BsEye />
                    </Link>
                    <Link
                        href={`/admin/kelas/${info.row.original.id}/edit`}
    ...
    const handleDeleteClick = (id) => {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                router.visit(`/admin/kelas/${id}`, { method: "delete" });
                Swal.fire({
                    title: "Deleted!",
        ...
        <div className="p-4 mx-auto fill-gray-400 bg-white shadow-md rounded-xl dark:bg-dots-lighter dark:bg-gray-800 selection:bg-red-500 selection:text-white ">
            <div className="flex justify-between">
                <div className="pb-2 text-gray-600 dark:text-white">
                    <p>Kelas X: {jumlahkelasX}</p>
                    <p>Kelas XI: {jumlahkelasXI}</p>
                    <p>Kelas XII: {jumlahkelasXII}</p>
                    <p>Total Kelas: {totalkelas}</p>
                </div>
            </div>
